...
[[results:statistics.json:design.arm_seed_evaluations]] arm–realisation evaluations,
[[results:statistics.json:design.scored_image_trials|,]] scored image trials). Prespecified
...
`d'` = [[results:dose_sweep.json:config.criteria.d_prime_threshold|.0f]]). Uncertainty is by
...
[[results:statistics.json:divergence.spearman_delta_ssim_delta_d_pw.ci_high|.2f]]), and
[[results:statistics.json:divergence.divergence_rate.value|.1%]]
([[results:statistics.json:divergence.divergence_rate.ci_low|.1%]] to
[[results:statistics.json:divergence.divergence_rate.ci_high|.1%]]) of processed evaluations
...
[[results:statistics.json:observer_dependence.overall.benefit.ci_high|+.2f]]), with mean
Δ`d'`(NPWE) = [[results:statistics.json:observer_dependence.overall.delta_d_npwe.value|+.2f]]
against Δ`d'`(PW) =
[[results:statistics.json:observer_dependence.overall.delta_d_pw.value|+.2f]], and
[[results:statistics.json:observer_dependence.npwe_improved_pw_did_not.value|.1%]] of
...
[[results:statistics.json:floor_strata["above floor"].task_degradation.value|.1%]] above the
floor against [[results:statistics.json:floor_strata["below floor"].task_degradation.value|.1%]]
...
rose by [[results:statistics.json:floor_strata["below floor"].mean_delta_ssim.value|+.3f]] in
...
[[results:statistics.json:ceiling.family_wise.n_processed_comparisons]] at a family-wise level
of `0.05`); the single exceedance
([[results:statistics.json:ceiling.violation_detail[0].excess|sci2]] in AUC) was an unprocessed
...
[[results:closed_form.json:max_relative_error|sci2]].
...
[[results:dose_sweep.json:config.criteria.d_prime_threshold|.0f]]. It is used in this paper
...
([[results:taskbench_cnn.json:rows[denoiser=CNN].kv|.0f]] kV,
[[results:taskbench_cnn.json:rows[denoiser=CNN].mas|.0f]] mAs). It produced the highest fidelity
of any processing in this study — SSIM
[[results:taskbench_cnn.json:rows[denoiser=CNN].ssim|.2f]] against
[[results:taskbench_cnn.json:rows[denoiser=raw].ssim|.2f]] for its input — with a prewhitening
`d'` of [[results:taskbench_cnn.json:rows[denoiser=CNN].d_prime_ideal|.2f]] against
[[results:taskbench_cnn.json:rows[denoiser=raw].d_prime_ideal|.2f]] on that input, contrast
recovery [[results:taskbench_cnn.json:rows[denoiser=CNN].contrast_recovery|.2f]], and no
